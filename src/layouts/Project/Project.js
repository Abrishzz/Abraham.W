  ...
  {header}
      ...
      <ul className={styles.meta}>
        {roles?.map((item, index) => (
          ...
          style={{...cssProps({ delay: numToMs(initDelay + 300 + index * 140) }), ...additionalStyles.metaItem}}
          key={item.role}
          >
            {item.icon && <Icon style={{ marginRight: '10px' }} icon={item.icon} />}
            <Text secondary>{item.role}</Text>
          </li>
        ))}
      </ul>
    )}
